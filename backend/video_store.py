...
class VideoStore:
    ...
    def ingest_video(
        ...
        for i in range(0, total, batch_size):
            ...
            contents = [
                types.Content(parts=[types.Part.from_bytes(data=self._resize_image(data), mime_type="image/jpeg")])
                for data, mime in batch
            ]
            result = self.client.models.embed_content(
                model=EMBED_MODEL,
    ...
    def delete_video(self, video_id: str) -> bool:
        """Delete all frames for a video and clean up disk files."""
        try:
            all_data = self.collection.get(where={"video_id": video_id}, include=["metadatas"])
            if all_data["ids"]:
                # Get the filename from the first frame's metadata to delete the video file
                filename = all_data["metadatas"][0]["video_filename"]
                ...
                video_file = Path("./uploads") / filename
